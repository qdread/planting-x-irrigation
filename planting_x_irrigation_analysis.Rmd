...
ggplot(cld_cfu_pooled, aes(x = PD, y = emmean, ymin = lower.CL, ymax = upper.CL, label = gsub(' ', '', .group))) +
  ...
  theme(legend.position = c(0.15, 0.2), legend.background = element_blank())
...
# This section currently not used.
yld_vs_audpc_lmm <- lmer(YldKgHa ~ log1p(AUDPC) * PD + (1|YR/Rep) + (1|Strains), data = dat)

xpred <- expm1(seq(log1p(0), log1p(107500), length.out = 101))
pred_dat <- CJ(AUDPC = xpred, PD = ordered(1:3))
...
ggplot(pred_dat, aes(x = AUDPC, y = YldKgHa, ymin = q025, ymax = q975, color = PD, fill = PD)) +
...
xpred <- expm1(seq(log1p(0), log1p(107500), length.out = 101))
pred_dat <- CJ(AUDPC = xpred, PD = ordered(1:3), Irr = factor(c('Irr','Nonirr')))
...
Summary statistics can be generated if needed.

###FIXME

- anova table for the regression
- make sure it's not bad to ignore genotype effect
- change limits on predicted data to only cover the observed data min and max
- maybe use dotted lines instead of ribbon
- write up what was done
